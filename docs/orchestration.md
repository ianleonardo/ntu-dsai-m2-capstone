...
│   ├── assets/
│   │   ├── __init__.py
│   │   ├── sec_direct_ingestion.py  # SEC data download -> BigQuery (direct)
│   │   └── dbt_integration.py       # dbt transformations (run/test)
│   ├── jobs/
│   │   ├── __init__.py
│   │   └── sec_pipeline_direct.py   # Pipeline orchestration jobs (direct)
│   ├── schedules/
│   │   ├── __init__.py
...
- `sec_direct_ingestion`: Downloads SEC data and loads directly into BigQuery.
...
- **`sec_pipeline_direct_complete_job`**: The full end-to-end SEC pipeline (Download → BigQuery → dbt → Summary).
...
1. Go to **Jobs** and select e.g., **sec_pipeline_direct_complete_job**.
...
Ensure `GOOGLE_APPLICATION_CREDENTIALS` is pointing to an active service account with BigQuery permissions (BigQuery Job User + BigQuery Data Editor, or broader).
